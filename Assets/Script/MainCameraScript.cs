...
public class MainCameraScript : MonoBehaviour {
    ...
    public void AdjustToLookAtTarget()
    {
        ...
        Vector3 normTarget = (target.position - transform.position).normalized;
        normTarget.y = 0;

        Vector3 normTrans = transform.forward;
        normTrans.y = 0;

        float dotR = Vector3.Dot(transform.right, normTarget);
        float dotL = Vector3.Dot(-transform.right, normTarget);
        if (dotR < dotL)
        {
            float angle = Vector3.Angle(normTrans, normTarget);
            currentX = Mathf.Lerp(currentX, currentX - Vector3.Angle(normTrans, normTarget), 1);            
        }
        else if (dotR > dotL)
        {
            float angle = Vector3.Angle(normTrans, normTarget);
